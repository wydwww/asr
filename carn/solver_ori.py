    ...
    def fit(self):
        ...
        while True:
            for inputs in self.train_loader:
                ...
                print("hr size: {}".format(hr.size())) 
                hr = hr.to(self.device)
                lr = lr.to(self.device)
                print("hr size after: {}".format(hr.size())) 
                sr = refiner(lr, scale)
                print("sr size: {}".format(sr.size()))
                #sr_save = sr.detach().squeeze(0)
                sr_save = sr[0].detach().squeeze(0)
                print("sr_save size: {}".format(sr_save.size()))
                sr_im_path = "/home/ubuntu/sr/CARN-pytorch/save/sr.png"
                save_image(sr_save, sr_im_path)
                save_image(hr[0], "/home/ubuntu/sr/CARN-pytorch/save/hr.png")
                save_image(lr[0], "/home/ubuntu/sr/CARN-pytorch/save/lr.png")
                print("saved!")
