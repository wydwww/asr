...
from dataset import TrainDatasetVisdrone as TrainDataset
...
class Solver():
    def __init__(self, model, cfg):
        ...
        self.writer = SummaryWriter(log_dir=os.path.join("runs", "visdrone", cfg.ckpt_name, str(cfg.scale)))
        if cfg.verbose:
            num_params = 0
...
#                if cfg.verbose and self.step % 100 == 0:
                    ...
                    #sr_im_path = "/home/ubuntu/sr/CARN-pytorch/save/sr_{}.png".format(self.step)
                    #save_image(sr_save, sr_im_path)
                    #save_image(hr[0], "/home/ubuntu/sr/CARN-pytorch/save/hr_{}.png".format(self.step))
                ...
                if cfg.verbose and self.step % cfg.print_interval == 0:
                    ...
                    if cfg.scale > 0:
                        ...
                        psnr = self.evaluate("dataset/visdrone", scale=cfg.scale, num_step=self.step)
                        self.writer.add_scalar("visdrone", psnr, self.step)
                    else:    
    ...
    def evaluate_6x(self, test_data_dir, scale=2, num_step=0):
        # This is an ugly workaround for a special case
        cfg = self.cfg
        mean_psnr = 0
        refiner = nn.DataParallel(self.refiner, 
                                  device_ids=range(cfg.num_gpu))
        ...
        for step, inputs in enumerate(test_loader):
            ...
            hr = hr.detach().cpu().mul(255).clamp(0, 255).byte().squeeze(0).permute(1, 2, 0).numpy()
            sr = sr.detach().cpu().mul(255).clamp(0, 255).byte().squeeze(0).permute(1, 2, 0).numpy()
            # evaluate PSNR
    ...
    def evaluate(self, test_data_dir, scale=2, num_step=0):
        with torch.no_grad():
            ...
            for step, inputs in enumerate(test_loader):
                ...
                hr = hr.detach().cpu().mul(255).clamp(0, 255).byte().permute(1, 2, 0).numpy()
                sr = sr.detach().cpu().mul(255).clamp(0, 255).byte().permute(1, 2, 0).numpy()
