...
class StorePreds:
    ...
    def __call__(self, pred, batch, additional):
        # batch is a list of tensor, batch size is 1
        b = self.to_img(batch)
        for p, im, gt, name, subset in zip(pred, b['image'], b['original_labels'], b['name'], b['subset']):
            # [im, self.to_color(p), gt] are [original image, pred map, pred map ground truth]
...
class StoreSubmissionPreds:
    ...
    def __call__(self, pred):
        for p in pred.astype(np.uint8):
            print(p.shape)
            pimg.fromarray(self.remap(p)).save(f'{self.store_dir}.png')
            pimg.fromarray(self.to_color(p)).save(f'{self.store_dir_color}.png')